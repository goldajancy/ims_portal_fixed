...
def admin_dashboard():
    ...
    for i in range(5, -1, -1):
        ...
        row = query(
        "SELECT COUNT(*) as cnt FROM users WHERE YEAR(created_at)=%s AND MONTH(created_at)=%s",
        (year_num, month_num), one=True
    )
    mon_label = datetime.date(year_num, month_num, 1).strftime('%%b')
    reg_trend.append({'mon': mon_label, 'cnt': row['cnt'] if row else 0})
...
def admin_send_attendance_link():
    ...
    if request.method == 'POST':
        ...
        return render_template('admin/send_attendance_link.html',
                               classes=classes, link=link, token=token)
    return render_template('admin/send_attendance_link.html', classes=classes, link=None)
